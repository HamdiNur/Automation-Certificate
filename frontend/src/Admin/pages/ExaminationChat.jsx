...
function ExaminationChat() {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        const res = await axios.get("http://localhost:5000/api/chat/department/exam_office", {
  ...
  useEffect(() => {
    ...
    socket.on("newMessage", (newMsg) => {
      if (newMsg.department !== "examination") return
      ...
      setConversations((prev) => {
        ...
        if (index !== -1) {
          ...
          updated[index] = {
            ...
            unreadCount:
              (!readTracker.current[studentId] ||
                new Date(newMsg.timestamp) > new Date(readTracker.current[studentId])) &&
            ...
            <div className="student-list">
              {loading ? (
                ...
                filteredConversations.map((conversation, index) => (
                  <div
                    key={index}
                    className={`student-item ${selectedStudent?.studentId === conversation.studentId ? "active" : ""}`}
                    onClick={() => {
...
export default ExaminationChat
