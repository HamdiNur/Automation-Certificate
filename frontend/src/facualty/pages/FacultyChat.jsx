...
function FacultyChat() {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        const res = await axios.get("http://localhost:5000/api/chat/department/faculty", {
    ...
    const saved = localStorage.getItem("facultyReadTracker")
  ...
  const saveReadTracker = () => {
    localStorage.setItem("facultyReadTracker", JSON.stringify(readTracker.current))
  }
  ...
  useEffect(() => {
    ...
    socket.on("newMessage", (newMsg) => {
      if (newMsg.department !== "faculty") return
