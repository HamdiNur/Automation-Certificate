...
unreadCount: (() => {
  ...
  // ✅ Count as unread only if it's newer AND you're not currently inside the chat
  const isUnread =
...
const groupMessagesByStudent = (messages) => {
  ...
  messages.forEach((msg) => {
    ...
    // Set latest message for display
    ...
    // ✅ Count unread ONLY if the message is from the student
    const lastRead = readTracker.current[studentId];
...
const handleReply = async () => {
  ...
  // ✅ Update last read time when opening a conversation
  readTracker.current[conversation.studentId] = new Date().toISOString();
            saveReadTracker(); // ✅ Add this line
