    ...
    <div className="dashboard-wrapper">
      ...
      <div className="dashboard-main">
        ...
        <div className="clearance-status">
          ...
          <ul>
            {group.clearanceProgress && Object.entries(group.clearanceProgress).map(([dept, status]) => (
              <li key={dept}>
                <strong>{dept}:</strong> {status.status} {status.date && `on ${new Date(status.date).toLocaleDateString()}`}
              </li>
            ))}
          </ul>
        </div>
